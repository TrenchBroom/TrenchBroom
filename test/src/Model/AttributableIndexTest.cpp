...
namespace TrenchBroom {
    namespace Model {
        static AttributableNodeArray findExactExact(const AttributableNodeIndex& index, const AttributeName& name, const AttributeValue& value) {
            return index.findAttributableNodes(AttributableNodeIndexQuery::exact(name), value);
        }
        
        static AttributableNodeArray findNumberedExact(const AttributableNodeIndex& index, const AttributeName& name, const AttributeValue& value) {
        ...
        TEST(EntityAttributeIndexTest, addAttributableNode) {
            ...
            AttributableNodeArray attributables = findExactExact(index, "test", "somevalue");
        ...
        TEST(EntityAttributeIndexTest, removeAttributableNode) {
            ...
            const AttributableNodeArray& attributables = findExactExact(index, "test", "somevalue");
            ASSERT_EQ(1u, attributables.size());
        ...
        TEST(EntityAttributeIndexTest, addNumberedEntityAttribute) {
            ...
            AttributableNodeArray attributables = findNumberedExact(index, "test", "somevalue");
            ASSERT_EQ(1u, attributables.size());
            ASSERT_TRUE(VectorUtils::contains(attributables, entity1));
        ...
        TEST(EntityAttributeIndexTest, addRemoveFloatProperty) {
            ...
            AttributableNodeArray attributables = findExactExact(index, "delay", "3.5");
            ASSERT_EQ(1u, attributables.size());
            ASSERT_TRUE(VectorUtils::contains(attributables, entity1));
