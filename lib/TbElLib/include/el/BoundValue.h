...
/**
 * A field-name -> accessor-function table for exposing a `T` as a `BoundValue`. Backed by
 * a flat_map rather than a std::map: these tables are small (a handful to a couple dozen
 * entries) and built once as a function-local static, so a sorted vector's cache locality
 * during lookup wins out over a tree's, without paying a hash table's per-element
 * allocation for a table that's never mutated after construction.
 */
template <typename T>
using BoundValueFields = kdl::flat_map<std::string, std::function<Value(const T&)>>;
