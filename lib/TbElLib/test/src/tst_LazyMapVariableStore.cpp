...
TEST_CASE("LazyMapVariableStore")
{
  ...
  SECTION("clone")
  {
    auto store = LazyMapVariableStore{makeLazyMapOf(std::string{"1"}, fields)};
  ...
  SECTION("size")
  {
    CHECK(LazyMapVariableStore{makeLazyMapOf(std::string{"1"}, fields)}.size() == 1u);
  }

  SECTION("value")
  {
    const auto store = LazyMapVariableStore{makeLazyMapOf(std::string{"1"}, fields)};
  ...
  {
    CHECK(
      LazyMapVariableStore{makeLazyMapOf(std::string{"1"}, fields)}.names()
