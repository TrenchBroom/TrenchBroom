...
TEST_CASE("PatchNodeLazyMap")
{
  ...
  SECTION("Owned by worldspawn")
  {
    ...
    const auto lazyMap = makePatchNodeLazyMap(map, *patchNode);
    ...
    CHECK(patchNodeFieldNames() == lazyMap.keys());

    CHECK(lazyMap.at("type") == el::Value{"patch"});
    CHECK(lazyMap.at("bounds") == el::Value{patchNode->logicalBounds()});
    CHECK(lazyMap.at("center") == el::Value{patchNode->logicalBounds().center()});
