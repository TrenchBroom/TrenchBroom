...
TEST_CASE("EntityNodeBinding")
{
  ...
  SECTION("makeEntityPropertiesBinding")
  {
    const auto value = makeEntityPropertiesBinding(entity);

    CHECK(value.typeName() == "Map");

    el::withEvaluationContext([&](auto&) {
      CHECK(value.boundValue().at("classname") == el::Value{"info_player_start"});
      CHECK(value.boundValue().at("missing") == std::nullopt);
      CHECK(
        value.boundValue().keys() == std::vector<std::string>{"classname", "targetname"});
    }).ignore();
  }

  SECTION("makeEntityBinding")
  {
    const auto value = makeEntityBinding(entity);
    ...
    el::withEvaluationContext([&](auto&) {
      ...
      CHECK(value.boundValue().at("missing") == std::nullopt);
      CHECK(
        value.boundValue().keys() == std::vector<std::string>{"classname", "properties"});

      const auto properties = value.boundValue().at("properties").value();
      CHECK(properties.boundValue().at("targetname") == el::Value{"start1"});
      CHECK(properties.boundValue().at("missing") == std::nullopt);
      CHECK(
        properties.boundValue().keys()
        == std::vector<std::string>{"classname", "targetname"});
    }).ignore();
  }

  SECTION("makeEntityNodeBinding")
  {
    ...
    const auto boundValue = makeEntityNodeBinding(map, *entityNode);
    ...
    CHECK(entityNodeFieldNames() == boundValue.keys());
