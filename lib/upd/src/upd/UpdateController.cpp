...
void UpdateController::setAutoCheckEnabled(const bool autoCheckEnabled)
{
  if (
    !std::exchange(m_autoCheckEnabled, autoCheckEnabled) && autoCheckEnabled
...
{
  if (m_autoCheckEnabled && std::holds_alternative<IdleState>(m_state))
  {
    checkForUpdates();
  }
  else
