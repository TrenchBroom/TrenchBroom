...
#include "gl/FontGlyph.h"
#include "gl/FontGlyphBuilder.h"
...
auto loadFont(
  FT_Library library, const FontDescriptor& fontDescriptor, const FindFontFunc& findFont)
{
  const auto fontPath = fontDescriptor.path().is_absolute()
                          ? fontDescriptor.path()
                          : findFont(fontDescriptor.path());
...
FreeTypeFontFactory::FreeTypeFontFactory(FindFontFunc findFontFunc)
  : m_findFontFunc{std::move(findFontFunc)}
...
{
  auto [face, bufferedReader] = loadFont(*m_library, fontDescriptor, m_findFontFunc);
