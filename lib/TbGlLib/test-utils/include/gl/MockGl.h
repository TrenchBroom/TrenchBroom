...
/**
 * Thrown by MockGl when a function is called for which no slot has been assigned.
 */
class MockGlUnexpectedCall : public std::runtime_error
{
public:
  explicit MockGlUnexpectedCall(std::string_view function);
};

/**
 * A Gl implementation for tests that need to observe or control calls to the Gl
 * interface. Each virtual function delegates to a public std::function slot named
 * `on<FunctionName>`. Assign a slot before a test exercises the corresponding call;
 * calling a function whose slot is empty throws MockGlUnexpectedCall.
 *
 * Use TestGl instead if the code under test does not need this level of control.
 */
class MockGl : public Gl
{
public:
  std::function<void(GLbitfield)> onClear;
  std::function<void(GLfloat, GLfloat, GLfloat, GLfloat)> onClearColor;

  std::function<void(GLint, GLint, GLsizei, GLsizei)> onViewport;

  std::function<void(GLenum)> onMatrixMode;
  std::function<void(const GLdouble*)> onLoadMatrixd;
  std::function<void(const GLfloat*)> onLoadMatrixf;
  ...
  std::function<void(GLenum)> onEnableClientState;
  std::function<void(GLenum)> onDisableClientState;

  std::function<void(GLbitfield)> onPushAttrib;
  std::function<void()> onPopAttrib;

  std::function<void(GLenum)> onEnable;
  std::function<void(GLenum)> onDisable;

  std::function<void(GLfloat)> onLineWidth;

  std::function<void(GLenum, GLenum)> onPolygonMode;

  std::function<void(GLenum)> onFrontFace;
  std::function<void(GLenum)> onCullFace;

  std::function<void(GLenum, GLenum)> onBlendFunc;

  std::function<void(GLenum)> onShadeModel;

  std::function<void(GLboolean)> onDepthMask;
  ...
  std::function<void(GLuint, GLsizei, const GLchar* const*, const GLint*)> onShaderSource;
  ...
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix2fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix3fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix4fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix2x3fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix3x2fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix2x4fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix4x2fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix3x4fv;
  std::function<void(GLint, GLsizei, GLboolean, const GLfloat*)> onUniformMatrix4x3fv;

  std::function<GLint(GLuint, const GLchar*)> onGetAttribLocation;
  std::function<GLint(GLuint, const GLchar*)> onGetUniformLocation;

  std::function<void(GLsizei, GLuint*)> onGenBuffers;
  std::function<void(GLsizei, const GLuint*)> onDeleteBuffers;

  std::function<void(GLenum, GLuint)> onBindBuffer;
  std::function<void(GLenum, GLsizeiptr, const GLvoid*, GLenum)> onBufferData;
  std::function<void(GLenum, GLintptr, GLsizeiptr, const void*)> onBufferSubData;

  std::function<void(GLint, GLenum, GLsizei, const GLvoid*)> onVertexPointer;
  std::function<void(GLint, GLenum, GLsizei, const GLvoid*)> onColorPointer;
  std::function<void(GLenum, GLsizei, const GLvoid*)> onNormalPointer;
  std::function<void(GLint, GLenum, GLsizei, const GLvoid*)> onTexCoordPointer;

  std::function<void(GLuint)> onEnableVertexAttribArray;
  std::function<void(GLuint)> onDisableVertexAttribArray;
  ...
  std::function<void(GLenum)> onActiveTexture;
  ...
  std::function<void(GLenum, GLfloat)> onPixelStoref;
  std::function<void(GLenum, GLint)> onPixelStorei;

  std::function<void(GLenum)> onClientActiveTexture;

  std::function<void(GLenum, GLint, GLsizei)> onDrawArrays;
  std::function<void(GLenum, GLsizei, GLenum, const void*)> onDrawElements;
  std::function<void(GLenum, const GLint*, const GLsizei*, GLsizei)> onMultiDrawArrays;

  std::function<const GLubyte*(GLenum)> onGetString;
  std::function<GLenum()> onGetError;
