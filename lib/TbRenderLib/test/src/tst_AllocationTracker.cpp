...
size_t getBrushSizeFromRandEngine(std::mt19937& engine)
{
 ...
 */
template <typename T>
void shuffle(std::vector<T>& vec, std::mt19937& engine)
{
...
TEST_CASE("AllocationTracker")
{
  ...
  SECTION("allocate and free")
  {
    SECTION("five allocations")
    {
      ...
      CHECK(
        t.usedBlocks() == (std::vector<AllocationTracker::Range>{{0, 100}, {100, 100}}));
      ...
      CHECK(
        t.freeBlocks()
        == (std::vector<AllocationTracker::Range>{{100, 100}, {300, 100}}));
      CHECK(t.largestPossibleAllocation() == 100u);

      // this will cause a merge with the left and right free blocks
      t.free(blocks[2]);
      CHECK(
        t.usedBlocks() == (std::vector<AllocationTracker::Range>{{0, 100}, {400, 100}}));
    ...
    SECTION("free merges with right neighbor")
    {
      ...
      t.free(blocks[1]);
      CHECK(
        t.usedBlocks() == (std::vector<AllocationTracker::Range>{{0, 100}, {300, 100}}));
      CHECK(t.freeBlocks() == (std::vector<AllocationTracker::Range>{{100, 200}}));

      CHECK(t.largestPossibleAllocation() == 200u);
    }

    SECTION("free merges with left neighbor")
    {
      AllocationTracker t(400);
  ...
  SECTION("expand")
  {
    ...
    SECTION("with used space at end")
    {
      AllocationTracker t(200);
  ...
  SECTION("benchmarks")
  {
    ...
    SECTION("allocate, free, allocate")
    {
      std::mt19937 randEngine;
