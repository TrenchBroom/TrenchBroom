...
TEST_CASE("Unzip")
{
  ...
  SECTION("fails when archive does not exist")
  {
    auto tempDir = QTemporaryDir{};
    REQUIRE(tempDir.isValid());
    ...
    CHECK(!unzip(zipPath, destPath, std::nullopt));
    CHECK(!QFileInfo{destPath}.exists());
  }

  SECTION("fails when destination path is a file")
  {
    const auto zipPath = findFixturePath("../fixture/unzip/archive.zip");
    auto tempDir = QTemporaryDir{};
    REQUIRE(tempDir.isValid());
  ...
  SECTION("fails for invalid archive content")
  {
    auto tempDir = QTemporaryDir{};
    REQUIRE(tempDir.isValid());
  ...
  SECTION("preserves executable permissions")
  {
  ...
  SECTION("preserves extended attributes on macOS")
  {
