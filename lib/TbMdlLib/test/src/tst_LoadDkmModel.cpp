...
TEST_CASE("loadDkmModel")
{
  auto logger = NullLogger{};
  auto fs = fs::VirtualFileSystem{};

  SECTION("valid DKM model")
  {
    const auto path = getFixtureRoot() / "test/mdl/LoadDkmModel/cube.dkm";
    const auto file = fs::Disk::openFile(path) | kdl::value();

    auto reader = file->reader().buffer();
    loadDkmModel("cube", reader, fs, logger) | kdl::transform([](const auto& modelData) {
  ...
  SECTION("invalid DKM file")
  {
    const auto path = getFixtureRoot() / "test/mdl/LoadDkmModel/invalid.dkm";
    const auto file = fs::Disk::openFile(path) | kdl::value();

    auto reader = file->reader().buffer();
    CHECK(
      loadDkmModel("cube", reader, fs, logger)
      == Result<mdl::EntityModelData>{Error{"Unknown DKM model ident: 305419896"}});
  }
}
