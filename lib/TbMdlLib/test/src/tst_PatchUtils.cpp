...
TEST_CASE("PatchUtils")
{
  SECTION("createPatch")
  {
    ...
    SECTION("n=6, irregular hexagon")
    {
      ...
      // The two patches must share the diagonal between (-208,-448,32) and
      // (-176,-320,32), not a boundary edge, so that together they exactly tile the
      // hexagonal face without overlapping or leaving a gap.
      CHECK(
  ...
  SECTION("resamplePatch")
  {
    ...
    {
      // Resampling at the same resolution is a degenerate case of exact subdivision:
      // every new sub-surface boundary coincides with an old one, so the fit must recover
      // the original control points (modulo floating point error from numeric quadrature)
    ...
    SECTION("more point counts refines the patch without changing its shape")
    {
      ...
      // precisely the same surface as the original at every parameter, not just at its
      // own control points.
    ...
    SECTION("fewer point counts still preserves the patch's corners exactly")
    {
      ...
      // quadratic sub-surfaces, so this fit is necessarily lossy, but it should still
      // stay visibly close to the original surface rather than diverge wildly.
      for (size_t i = 0; i <= 10; ++i)
      {
        const auto u = double(i) / 10.0;
        for (size_t j = 0; j <= 10; ++j)
        {
          ...
          CAPTURE(u, v);
          CHECK(
            vm::approx{resampled.evaluateAt(u, v), 1.0} == largePatch.evaluateAt(u, v));
        }
      }
    }
    ...
    SECTION(
      ...
      "boundaries")
    {
      // The old patch has 3 column sub-surfaces (boundaries at 1/3 and 2/3) but only 1
      // row sub-surface, so the row direction stays unchanged while resampling to 2
      // column sub-surfaces forces each new column sub-surface to fit against exactly one
      // of the two old boundaries while excluding the other (the new boundary at u = 0.5
      // lies between them), exercising the boundary-splitting logic that the other
      // sections above never reach.
      ...
      // u = 0 and u = 1 are the patch's own domain boundaries, so every row's fitted
      // curve must reduce to the original boundary control point there, modulo floating
      // point error from the quadrature-based fit (only rows 0 and 2 are exact corners,
      // computed by direct surface evaluation; row 1 is itself fitted, so it can only
      // match approximately).
      ...
      // The two new edge midpoints (u = 0.25 and u = 0.75) are independently computed
      // below by exact symbolic integration of the same L2 fit, deliberately not reusing
      // the production code's own quadrature. The new boundary at u = 0.5 sits strictly
      // between the old boundaries at 1/3 and 2/3, so the [0, 0.5] fit must include only
      // the 1/3 boundary and the [0.5, 1] fit must include only the 2/3 boundary; using
      // the wrong sub-surface count (e.g. mixing up rows and columns) would silently skip
      // that split and miss this exact value while staying within the very loose
      // surface-closeness tolerance used below.
      for (const auto row : {size_t{0}, size_t{2}})
      {
