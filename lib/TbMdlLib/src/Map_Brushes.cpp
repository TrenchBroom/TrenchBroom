...
void justifyUV(
  Map& map, const UvAxis uvAxis, const UvDirection uvDirection, const UvPolicy uvPolicy)
{
  auto& selection = map.selection();
  contract_assert(selection.brushFaces.size() == 1);

  const auto& brushFace = selection.brushFaces.front().face();
  setBrushFaceAttributes(map, mdl::justify(brushFace, uvAxis, uvDirection, uvPolicy));
}
...
void fitUV(Map& map, const UvAxis uvAxis, const UvPolicy uvPolicy)
{
  auto& selection = map.selection();
  contract_assert(selection.brushFaces.size() == 1);
  ...
  const auto invariantVertex = justifiedVertex(faceHandle.face(), uvAxis);
