...
private: // subclassing interface - these will be called in the order that nodes should be
   ...
   */
  virtual Node* onWorldNode(
    std::unique_ptr<WorldNode> worldNode, ParserStatus& status) = 0;

  /**
   * Called for each custom layer.
   */
  virtual void onLayerNode(std::unique_ptr<Node> layerNode, ParserStatus& status) = 0;

  /**
   * Called for each group, entity entity or brush node. The given parent can be null.
   */
  virtual void onNode(
    Node* parentNode, std::unique_ptr<Node> node, ParserStatus& status) = 0;

  /**
   * Called for each brush face.
   */
  virtual void onBrushFace(BrushFace face, ParserStatus& status);
};
