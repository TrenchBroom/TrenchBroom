...
TEST_CASE("QueryBuilder::buildQueryExpression")
{
  ...
  {
    CHECK(
      textOf({}, QueryCombinator::All, ql::QueryObjectType::Layer)
      == R"(type == "layer")");

    const auto emptyCondition =
      QueryCondition{"classname", "", QueryConditionOperator::Equals, ""};
    CHECK(
      textOf({emptyCondition}, QueryCombinator::All, ql::QueryObjectType::Layer)
      == R"(type == "layer")");
  }
  ...
  SECTION("a hint is ANDed onto a single row without extra parens")
  {
    ...
    CHECK(
      textOf({condition}, QueryCombinator::Any, ql::QueryObjectType::Entity)
  ...
  SECTION("a hint is ANDed onto an All-chain without extra parens")
  {
    ...
    CHECK(
      textOf(conditions, QueryCombinator::All, ql::QueryObjectType::Entity)
  ...
  SECTION("a hint parenthesizes an Any-chain of more than one row")
  {
    ...
    CHECK(
      textOf(conditions, QueryCombinator::Any, ql::QueryObjectType::Entity)
      == R"(type == "entity" && ( classname == "x" || visible == true ))");
  }
}
...
TEST_CASE("QueryBuilder::tryImportQuery")
{
  SECTION("round-trips every shape buildQueryExpression can produce")
  {
    ...
    struct TestCase
    {
      ...
      std::optional<ql::QueryObjectType> hint;
    };

    const auto testCase = GENERATE(
      TestCase{QueryCombinator::All, std::nullopt},
      TestCase{QueryCombinator::Any, std::nullopt},
      TestCase{QueryCombinator::All, std::optional{ql::QueryObjectType::Entity}},
      TestCase{QueryCombinator::Any, std::optional{ql::QueryObjectType::Entity}});
  ...
  SECTION("round-trips a single row with a hint")
  {
    const auto condition =
      QueryCondition{"classname", "", QueryConditionOperator::Equals, "x"};
    const auto text =
      textOf({condition}, QueryCombinator::All, ql::QueryObjectType::Entity);

    const auto imported = importText(text);
    REQUIRE(imported.has_value());
    CHECK(imported->hint == ql::QueryObjectType::Entity);
  ...
  {
    const auto imported =
      importText(textOf({}, QueryCombinator::All, ql::QueryObjectType::Layer));
    REQUIRE(imported.has_value());
    CHECK(imported->hint == ql::QueryObjectType::Layer);
