...
el::ExpressionNode typeHintNode(const ql::QueryKind kind)
{
...
std::optional<ql::QueryKind> matchTypeHint(const el::ExpressionNode& node)
{
  return unwrapGroup(node).accept(kdl::overload(
    [](const el::BinaryExpression& b) -> std::optional<ql::QueryKind> {
      ...
      if (isVariableNamed(b.leftOperand, "type"))
      {
        ...
        {
          return ql::queryKindFromName(*name);
        }
      }
    ...
    [](const auto&) -> std::optional<ql::QueryKind> { return std::nullopt; }));
}
