    ...
    namespace VecMath {
        ...
        public:
            ...
            void testNonParallelPlane() {
                ...
                plane = Planef(Vec3f(0.8f, 0.0f, 1.0f).normalized(), 0.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);

                plane = Planef(Vec3f(0.8f, 0.0f, 1.0f).normalized(), 0.7f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.8f, 0.4f, 1.0f).normalized(), 189.23222f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.636535f, 0.702198f, 0.318969f).normalized(), 72.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.905819f, 0.423666f, 0.000290979f).normalized(), 72.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.98036f, 0.19719f, 0.00319336f).normalized(), 1406.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.514331f, 0.857591f, 0.000837219f).normalized(), 635.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.515365f, 0.606079f, 0.60586f).normalized(), 1830.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.0449349f, 0.706393f, 0.706393f).normalized(), 815.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(std::abs(plane.distance - test.distance) <= 1.0f);
                
                plane = Planef(Vec3f(0.994042f, 0.086082f, 0.0668672f).normalized(), 1594.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));
                
                plane = Planef(Vec3f(0.3185f, 0.000606089f, 0.947923f).normalized(), 224.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));
                
                plane = Planef(Vec3f(0.990495f, 0.0042303f, 0.137485f).normalized(), 1706.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));
                
                plane = Planef(Vec3f(835.0f, 825.0f, 3703.0f).normalized(), 1861.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));
                
                plane = Planef(Vec3f(625.0f, 1418.0f, 1418.0f).normalized(), 1630.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));
                
                plane = Planef(Vec3f(1424.0f, 2160.0f, 2160.0f).normalized(), 442.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));
                ...
                plane = Planef(Vec3f(3998.0f, 3998.0f, 1948.0f).normalized(), 839.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));

                plane = Planef(Vec3f(2522.0f, 1.0f, 1600.0f).normalized(), 1906.0f);
                findPoints(plane, points);
                assert(test.setPoints(points[0], points[1], points[2]));
                assert(test.normal.dot(plane.normal) > 0.99f);
                assert(Math<float>::lte(std::abs(plane.distance - test.distance), 1.0f));
            ...
            void testRandomPlanes() {
                ...
                for (size_t i = 0; i < NumPlanes; i++) {
                    ...
                    plane = Planef(Vec3f(x, y, z).normalized(), d);
                    findPoints(plane, points);
                    assert(test.setPoints(points[0], points[1], points[2]));
                    ...
                    assert(normalError < Math<float>::radians(1.0f));
                    assert(Math<float>::lte(distanceError, 2.0f));
                }
                ...
                std::cout << "Normal error min: " << Math<float>::degrees(minNormalError) << " max: " << Math<float>::degrees(maxNormalError) << " avg: " << Math<float>::degrees(avgNormalError) << std::endl;
