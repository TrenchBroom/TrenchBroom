...
#include "MoveObjectToolFigure.h"
...
namespace TrenchBroom {
    namespace Renderer {
        MoveObjectToolFigure::MoveObjectToolFigure(Controller::MoveObjectTool& moveObjectTool) : m_moveObjectTool(moveObjectTool), m_guideFigure(NULL) {}
        
        MoveObjectToolFigure::~MoveObjectToolFigure() {
        ...
        void MoveObjectToolFigure::render(RenderContext& context, Vbo& vbo) {
            ...
            if (!m_moveObjectTool.checkFigureDataValid()) {
                BBox bounds = m_moveObjectTool.editor().map().selection().bounds();
