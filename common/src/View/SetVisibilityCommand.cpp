...
namespace TrenchBroom {
    namespace View {
        ...
        SetVisibilityCommand::Ptr SetVisibilityCommand::show(const Model::NodeArray& nodes) {
            return Ptr(new SetVisibilityCommand(nodes, Action_Show));
        }
        
        SetVisibilityCommand::Ptr SetVisibilityCommand::hide(const Model::NodeArray& nodes) {
            return Ptr(new SetVisibilityCommand(nodes, Action_Hide));
        }
        
        SetVisibilityCommand::Ptr SetVisibilityCommand::ensureVisible(const Model::NodeArray& nodes) {
            return Ptr(new SetVisibilityCommand(nodes, Action_Ensure));
        }

        SetVisibilityCommand::Ptr SetVisibilityCommand::reset(const Model::NodeArray& nodes) {
            return Ptr(new SetVisibilityCommand(nodes, Action_Reset));
        }

        SetVisibilityCommand::SetVisibilityCommand(const Model::NodeArray& nodes, const Action action) :
