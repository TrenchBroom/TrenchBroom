 ...
 *
 * Since including glew before QOpenGLWidget only generates a warning and does not seem to incur any
 * ill effects, we silence the warning here.
 *
 * Note that GCC does not let us silence this warning using diagnostic pragmas, so it is disabled in
 * the CXX_FLAGS!
 */
...
RenderView::RenderView(GLContextManager& contextManager, QWidget* parent)
  ...
  connect(fpsCounter, &QTimer::timeout, [&]() {
    ...
    m_currentFPS = std::string("Avg FPS: ") + std::to_string(avgFps) +
...
static auto mouseEventWithFullPrecisionLocalPos(const QWidget* widget, const QMouseEvent* event) {
  ...
  return QMouseEvent(
    event->type(), localPos, event->windowPos(), event->screenPos(), event->button(),
...
void RenderView::renderFocusIndicator() {
  ...
  const auto projection =
    vm::ortho_matrix(-1.0f, 1.0f, 0.0f, 0.0f, static_cast<float>(w), static_cast<float>(h));
  ...
  auto array = Renderer::VertexArray::move(std::vector<Vertex>(
    {// top
     Vertex(vm::vec3f(0.0f, 0.0f, 0.0f), outer), Vertex(vm::vec3f(w, 0.0f, 0.0f), outer),
     Vertex(vm::vec3f(w - t, t, 0.0f), inner), Vertex(vm::vec3f(t, t, 0.0f), inner),

     // right
     Vertex(vm::vec3f(w, 0.0f, 0.0f), outer), Vertex(vm::vec3f(w, h, 0.0f), outer),
     Vertex(vm::vec3f(w - t, h - t, 0.0f), inner), Vertex(vm::vec3f(w - t, t, 0.0f), inner),

     // bottom
     Vertex(vm::vec3f(w, h, 0.0f), outer), Vertex(vm::vec3f(0.0f, h, 0.0f), outer),
     Vertex(vm::vec3f(t, h - t, 0.0f), inner), Vertex(vm::vec3f(w - t, h - t, 0.0f), inner),

     // left
     Vertex(vm::vec3f(0.0f, h, 0.0f), outer), Vertex(vm::vec3f(0.0f, 0.0f, 0.0f), outer),
     Vertex(vm::vec3f(t, t, 0.0f), inner), Vertex(vm::vec3f(t, h - t, 0.0f), inner)}));
