 ...
 * command history in the form of a stack of undo commands and a stack of redo commands.
 *
 * Successive commands can be collated if they meet certain conditions. For two commands to be
 * collated, the following conditions must apply:
 *
 ...
 *
 * The command processor supports nested transactions. Each transaction can be committed or rolled
 * back individually. Committing a nested transaction adds it as a command to the containing
 * transaction.
 */
...
public:
  ...
  /**
   * Starts a new transaction. If a transaction is currently executing, then the newly started
   * transaction becomes a nested transaction and will be added as a command to its parent
   ...
   * will be added to the containing transaction as a single command.
   *
   * If the current transaction does not contain any commands, then the transaction ends, but
   * nothing will be stored in the command processor.
   *
  ...
  /**
   * Rolls the currently executing transaction back by undoing all commands that belong to the
   * transaction. The transaction does not end when it is rolled back, rather, it remains executing.
   * To end a transaction after it was rolled back, call `commitTransaction`. Since the transaction
   * will be empty, committing it will just do nothing but remove the transaction itself.
   *
   * @throws CommandProcessorException if no transaction is currently executing
   */
   ...
   * commands on these stacks.
   *
   * The command processor takes ownership of the given command, and since it is not stored, it will
   * be deleted immediately after execution.
   *
  ...
  /**
   * Executes the given command by calling its `performDo` method and stores it for later undo if it
   * is successfully executed. In that case, the redo stack is cleared since the application's state
   * is likely to have become inconsistent with the state expected by the commands on the redo
   * stack.
   *
   * If the given command is executed successfully, the command processor will attempt to collate it
   * with the command at the top of the undo stack by calling its `collateWith` method. If the
   * collation takes place, then the given command will not be stored on the undo stack.
   *
   * The command processor takes ownership of the given command, so unless it is stored on the undo
   * stack under the previously described conditions, the command is deleted immediately after it is
   * executed.
   *
   * @param command the command to execute
   * @return the result of executing the given command
  ...
  /**
   * Undoes the most recently executed command by calling its `performUndo` method and stores the
   * command on the redo stack if it could be undone successfully, i.e. its `performUndo` method
   * returned a successful command result.
   *
   * @return the result of undoing the command
   *
   * @throws CommandProcessorException if a transaction is currently being executed or if the undo
   * stack is empty
   */
  std::unique_ptr<CommandResult> undo();

  /**
   * Redoes the most recently undone comment by calling its `performDo` method and stores the
   * command on the undo stack if it could be executed successfully, i.e. its `performDo` method
   * returned a successful command result.
   *
   * @return the result of executing the command
   *
   * @throws CommandProcessorException if a transaction is currently being executed or if the redo
   * stack is empty
   */
  std::unique_ptr<CommandResult> redo();

  /**
   * Clears this command processor. Both the undo and the redo stack are cleared, and therefore all
   * stored commands are deleted as well.
   */
  void clear();

private:
  /**
   * Executes and stores the given command. The command will only be stored if it was executed
   * successfully and if it wasn't collated with the topmost command on the undo stack.
   *
   * @param command the command to execute and store
   * @param collate whether or not the given command should be collated with the topmost command on
   * the undo stack
   * @return a struct containing the result of executing the given command and a boolean indicating
   * whether the given command was stored on the undo stack
   */
   ...
   * @param collate whether not to attempt to collate the given command with the topmost command on
   * the undo stack
  ...
  /**
   * Pushes the given command to the back of the list of commands belonging to the currently
   * executing transaction. If `collate` is `true`, then it is attempted to collate the given
   * command with the last command executed in the currently executing transaction.
   *
   * Precondition: a transaction is currently executing
   *
   * @param command the command to push
   * @param collate whether or not to attempt to collate the given command with the last command
   * executed in the currently executing transaction
   ...
   *
   * Triggers a `transactionDone` notification. Afterwards, the transaction is stored as a single
   * command on the undo stack.
   */
  void createAndStoreTransaction();

  /**
   * Creates and returns a command that, when executed, executes all commands which were stored in
   * the scope of the current transaction. When this command is undone, it also undoes all of these
   ...
   * command on the undo stack. Takes ownership of the given command, so if it isn't stored on the
   * undo stack, the command is deleted.
   *
   * @param command the command to push
   * @param collate whether or not it should be attempted to collate the given command with the
   * topmost command on the undo stack
