...
namespace TrenchBroom {
    namespace View {
        ...
        void ScaleObjectsToolPage::setAxis(const Math::Axis::Type axis) {
            m_axis->SetSelection(static_cast<int>(axis));
        }

        void ScaleObjectsToolPage::setRecentlyUsedCenters(const Vec3::List& centers) {
        ...
        void ScaleObjectsToolPage::setCurrentCenter(const Vec3& center) {
        ...
        void ScaleObjectsToolPage::createGui() {
            ...
            Bind(wxEVT_IDLE, &ScaleObjectsToolPage::OnIdle, this);
            m_recentlyUsedCentersList->Bind(wxEVT_TEXT_ENTER, &ScaleObjectsToolPage::OnCenterChanged, this);
            m_recentlyUsedCentersList->Bind(wxEVT_COMBOBOX, &ScaleObjectsToolPage::OnCenterChanged, this);
            m_resetCenterButton->Bind(wxEVT_BUTTON, &ScaleObjectsToolPage::OnResetCenter, this);
            m_angle->Bind(SPIN_CONTROL_EVENT, &ScaleObjectsToolPage::OnAngleChanged, this);
            m_rotateButton->Bind(wxEVT_UPDATE_UI, &ScaleObjectsToolPage::OnUpdateRotateButton, this);
            m_rotateButton->Bind(wxEVT_BUTTON, &ScaleObjectsToolPage::OnRotate, this);
        ...
        void ScaleObjectsToolPage::OnIdle(wxIdleEvent& event) {
        ...
        void ScaleObjectsToolPage::OnCenterChanged(wxCommandEvent& event) {
            if (IsBeingDeleted()) return;
        ...
        void ScaleObjectsToolPage::OnResetCenter(wxCommandEvent& event) {
            if (IsBeingDeleted()) return;
            
            m_tool->resetRotationCenter();
        }
        
        void ScaleObjectsToolPage::OnAngleChanged(SpinControlEvent& event) {
            if (IsBeingDeleted()) return;
        ...
        void ScaleObjectsToolPage::OnUpdateRotateButton(wxUpdateUIEvent& event) {
            if (IsBeingDeleted()) return;
        ...
        void ScaleObjectsToolPage::OnRotate(wxCommandEvent& event) {
            if (IsBeingDeleted()) return;
        ...
        Vec3 ScaleObjectsToolPage::getAxis() const {
