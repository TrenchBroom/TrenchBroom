...
MouseEvent::MouseEvent(
  const Type i_type, const Button i_button, const WheelAxis i_wheelAxis, const float i_posX,
  const float i_posY, const float i_scrollDistance)
  : type(i_type)
  , button(i_button)
...
void InputEventQueue::processEvents(InputEventProcessor& processor) {
  ...
  // menu), then stale events maybe processed again.
...
void InputEventRecorder::recordEvent(const QMouseEvent& qEvent) {
  ...
  if (type == MouseEvent::Type::Down) {
    ...
    // (Implemented ourselves rather than using Qt's implementation to work around Qt bug, see
    // Main.cpp)
    ...
    if (m_dragging) {
      ...
      if (duration < minDuration) {
        ...
        if (!isDrag(posX, posY)) {
          m_queue.enqueueEvent(std::make_unique<MouseEvent>(
            MouseEvent::Type::Click, button, wheelAxis, m_lastClickX, m_lastClickY,
            scrollDistance));
        }
      } else {
        m_queue.enqueueEvent(std::make_unique<MouseEvent>(
    ...
    if (!m_dragging && m_anyMouseButtonDown) {
      if (isDrag(posX, posY)) {
        m_queue.enqueueEvent(std::make_unique<MouseEvent>(
          MouseEvent::Type::DragStart, button, wheelAxis, m_lastClickX, m_lastClickY,
          scrollDistance));
        m_dragging = true;
      }
    }
...
void InputEventRecorder::recordEvent(const QWheelEvent& qtEvent) {
  // These are the mouse X and Y position, not the wheel delta, in points relative to top left of
  // widget.
  ...
  if (scrollDistance.x() != 0.0f) {
    m_queue.enqueueEvent(std::make_unique<MouseEvent>(
      MouseEvent::Type::Scroll, MouseEvent::Button::None, MouseEvent::WheelAxis::Horizontal, posX,
      posY, static_cast<float>(scrollDistance.x())));
  }
  if (scrollDistance.y() != 0.0f) {
    m_queue.enqueueEvent(std::make_unique<MouseEvent>(
      MouseEvent::Type::Scroll, MouseEvent::Button::None, MouseEvent::WheelAxis::Vertical, posX,
      posY, static_cast<float>(scrollDistance.y())));
  }
}
