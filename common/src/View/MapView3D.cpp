             ...
             [&](auto&& thisLambda, Model::EntityNode* entity) {
               if (!entity->hasChildren()) {
                 for (size_t i = 0u; i < 4u; ++i) {
                   entity->logicalBounds().for_each_vertex([&](const auto& point) {
                     handlePoint(point, frustumPlanes[i]);
                   });
                 }
               } else {
                 entity->visitChildren(thisLambda);
...
void MapView3D::animateCamera(
  const vm::vec3f& position, const vm::vec3f& direction, const vm::vec3f& up, const int duration) {
