    ...
    namespace View {
        ...
        QVariant KeyboardShortcutModel::data(const QModelIndex& index, const int role) const {
            ...
            if (role == Qt::DisplayRole || role == Qt::EditRole) {
                ...
                if (index.column() == 0) {
                    const auto preference = Preference<KeyboardShortcut>(action.preferencePath, KeyboardShortcut(action.defaultKey));
        ...
        bool KeyboardShortcutModel::setData(const QModelIndex& index, const QVariant& value, const int role) {
            ...
            auto preference = Preference<KeyboardShortcut>(action.preferencePath, KeyboardShortcut(action.defaultKey));
            auto& prefs = PreferenceManager::instance();
            prefs.set(preference, KeyboardShortcut(value.value<QKeySequence>()));
