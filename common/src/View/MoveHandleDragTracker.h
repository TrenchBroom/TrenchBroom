...
namespace TrenchBroom {
    namespace View {
        ...
        /**
         * The move tracker's delegate. Provides callbacks which can be overridden to react to the different events that
         * can arise.
         */
        struct MoveHandleDragTrackerDelegate {
             ...
             * proposed handle position. Return this value if the move can be applied without error.
             * If this function returns DragStatus::Deny, the current handle position is kept and the move continues.
             * Return this value if the move cannot be applied to the object being moved.
             * If this functions returns DragStatus::End, the end function is called and the move ends. Return
             * this value if the move cannot continue, i.e. because the object being moved was removed.
             *
            ...
            /**
             * Called when the move ends successfully, i.e. if the move callback returned DragStatus::End or if the user
            ...
            /**
             * Called when the move is cancelled, i.e. if the user hit the escape key, or if the window loses focus.
             *
             * @param dragState the current drag state
             */
            ...
            /**
             * Returns a handle snapper. This is called once when the move start and when a modifier key is pressed or
             * released such that the move direction or snap mode changes.
             *
         ...
         *
         * This is a drag delegate for HandleDragTracker, but its behavior can be implemented and adapted by providing
         * it with its own delegate derived from MoveHandleDragTrackerDelegate.
         *
         * This drag delegate provides the following behavior:
         *
         * === In 3D Views ===
         *
         * By default, objects are dragged on a horizontal plane. Using the Alt key, the user can switch to moving on a
         * vertical line during a move. Holding shift can restrict a move to one axis in a horizontal move. Holding the
         * Ctrl (Cmd on macOS) switches the snap mode between relative and absolute snapping, if supported by the
         * delegate.
         *
         * === In 2D Views ===
         *
         * By default, objects are dragged on a plane that is orthogonal to the coordinate system axis that best matches
         * the camera's view direction. If this is the X axis, then the move happens on the Y/Z plane, with the camera
         * looking along the negative X axis. If the best matching axis is the Y axis, then the move happens on the X/Z
         * plane, with the camera looking along the positive Y axis. Otherwise, the move happens on the X/Y plane, with
         * the camera looking towards the negative Z axis.
         *
         * +Z ^              +Z ^              +Y ^
         *    |                 | +Y              |
         *    |                 |/                |
         ...
         *  +X                                  +Z
         *
         * Like in the 3D views, holding shift can restrict the move to one axis, and Ctrl (Cmd on macOS) switches
         * between relative and absolute snapping if supported. The Alt key has no effect in 2D views.
         *
         * In both view types, this delegate renders a move trace while a move is in progress. The move trace is a set
         * of lines parallel to the coordinate system axes (one for each axes). It illustrates the total movement of
         * the handle being moved.
         */
        ...
        public:
             ...
             * MoveHandleDragTrackerDelegate and is used to implement the actual effects and refine the behavior of this
             * delegate.
             */
            ...
            HandlePositionProposer start(const InputState& inputState, const vm::vec3& initialHandlePosition, const vm::vec3& handleOffset) override {
                ...
                auto dragHandlePicker = verticalMove
            ...
            std::optional<UpdateDragConfig> modifierKeyChange(const InputState& inputState, const DragState& dragState) override {
                ...
                if (nextMoveMode != m_lastMoveMode) {
                    const auto resetInitialHandlePosition = (m_lastMoveMode == MoveMode::Vertical
                        ? ResetInitialHandlePosition::Reset
                ...
                if (dragState.currentHandlePosition != dragState.initialHandlePosition) {
                    ...
                    for (size_t i = 0; i < 3; ++i) {
                        ...
                        if (stage != vm::vec3::zero()) {
                            ...
                            renderService.renderString(str, Renderer::SimpleTextAnchor{vm::vec3f{midPoint}, Renderer::TextAlignment::Bottom});
        ...
        /**
         * Creates a new handle drag tracker that uses a MoveHandleDragDelegate, which in turn uses the given delegate.
         */
        template <typename Delegate>
        std::unique_ptr<HandleDragTracker<MoveHandleDragDelegate<Delegate>>> createMoveHandleDragTracker(Delegate delegate, const InputState& inputState, const vm::vec3& initialHandlePosition, const vm::vec3& handleOffset) {
            return std::make_unique<HandleDragTracker<MoveHandleDragDelegate<Delegate>>>(MoveHandleDragDelegate{std::move(delegate)}, inputState, initialHandlePosition, handleOffset);
        }
