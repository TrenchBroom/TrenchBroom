...
static std::pair<AnchorPos, ProportionalAxes> modifierSettingsForInputState(
  ...
  const auto centerAnchor =
    inputState.modifierKeysDown(ModifierKeys::MKAlt) ? AnchorPos::Center : AnchorPos::Opposite;
...
static void renderDragSideHighlights(
  ...
  for (const auto& side : sides) {
    ...
    // In 2D, additionally stroke the edges of this polyhedron, so it's visible even when looking at
    // it from an edge
    if (renderContext.camera().orthographicProjection()) {
      auto renderService = Renderer::RenderService{renderContext, renderBatch};
      renderService.setLineWidth(2.0);
      renderService.setForegroundColor(
        Color(pref(Preferences::ScaleOutlineColor), pref(Preferences::ScaleOutlineDimAlpha)));
