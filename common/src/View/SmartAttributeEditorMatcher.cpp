...
namespace TrenchBroom {
    namespace View {
        ...
        bool SmartAttributeEditorMatcher::matches(const Model::AttributeName& name, const Model::AttributableList& attributables) const {
        ...
        bool SmartAttributeEditorKeyMatcher::doMatches(const Model::AttributeName& name, const Model::AttributableList& attributables) const {
            return !attributables.empty() && m_names.count(name) > 0;
        }

        bool SmartAttributeEditorDefaultMatcher::doMatches(const Model::AttributeName& name, const Model::AttributableList& attributables) const {
            return true;
        }
    }
}
