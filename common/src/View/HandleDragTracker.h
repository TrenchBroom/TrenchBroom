 ...
 *
 * If this function returns nullopt, the drag will continue, but the delegate's drag callback will
 * not be called.
 */
...
/**
 * Returned from the delegate's modifierKeyChange callback. The tracker's handle position mapping
 * function is updated with proposeHandlePosition, and if resetInitialHandlePosition is set to
 * Reset, the drag state's initial handle position is updated to the current handle position.
 */
...
enum class DragStatus
{
   ...
   * dragged. The current handle position is not updated in this case. */
  Deny,
...
/**
 * The drag tracker's delegate. This provides callbacks which can be overridden to react to the
 * different events that can arise during a drag.
 */
struct HandleDragTrackerDelegate {
   ...
   * function should be used to update the object being dragged.
   *
   * If this function returns DragStatus::Continue, the current handle position is replaced by the
   * new proposed handle position. Return this value if the drag can be applied without error. If
   * this function returns DragStatus::Deny, the current handle position is kept and the drag
   * continues. Return this value if the drag cannot be applied to the object being dragged. If this
   * functions returns DragStatus::End, the end function is called and the drag ends. Return this
   * value if the drag cannot continue, i.e. because the object being dragged was removed.
   *
  ...
  /**
   * Called when the drag ends successfully, i.e. if the drag callback returned DragStatus::End or
  ...
  /**
   * Called when the drag is cancelled, i.e. if the user hit the escape key, or if the window loses
   * focus.
   *
   * @param dragState the current drag state
   */
  ...
  /**
   * Called when any modifier key is pressed or released. Can be overridden to update the function
   * that the drag tracker uses to compute proposed handle positions. For example, a tool might
   * change how the handle position is snapped mid drag when a modifier key is pressed.
   *
   * If this function returns nullopt, the current handle proposer and initial handle position is
   * kept.
   *
   * @param inputState the current inputState
   * @param dragState the current drag state
   * @return a struct with a new function to propose handle positions, and an instruction whether or
   * not to update the initial handle position
   */
 ...
 *
 * In this context, a handle is a 3D point. This drag tracker keeps track of the initial handle
 * position and the current handle position. The initial handle position is the position that was
 * passed to the constructor. It can be updated if the drag mode changes in response to a modifier
 * key change.
 *
 ...
 *
 * The delegate's start function is called once when this drag tracker is constructed. It must
 * return the handle proposer function to use initially. The delegate's modifierKeyChange function
 * can optionally return a new handle proposer function and it can instruct the tracker to update
 * the initial handle position. This can be used to change the characteristics of the drag in
 * response to a modifier key change. For example, in a 3D view, the user may hold a modifier key to
 * switch between dragging horizontally and vertically.
 */
...
public:
  ...
  /**
   * React to modifier key changes. This is delegated to the delegate, and if it returns a new
   * handle position proposer function, the drag tracker's proposer function is replaced.
   * Optionally, the initial handle position is updated according to the value of the returned
   * ResetInitialHandlePosition value.
   *
   * If a new proposer function is returned by the delegate, it is called with the current drag
   * state and drag() is called with the new proposed handle position.
   */
  ...
  /**
   * Called when the mouse is moved during a drag. Delegates to the delegate to apply changes to the
   * objects being dragged.
   *
   * Returns true to indicate succes. If this function returns false, the drag ends and end() is
   * called.
   */
  ...
  /**
   * Called when the drag ends normally (e.g. by releasing a mouse button) or if drag() returns
   * false. The delegate should commit any changes made in result of the drag.
   */
  void end(const InputState& inputState) override { m_delegate.end(inputState, m_dragState); }

  /**
   * Called when the drag ends abnormally (e.g. by hitting escape during a drag). The delegate
   * should undo any changes made in result of the drag.
   */
...
private:
  bool drag(const InputState& inputState, const IdenticalPositionPolicy identicalPositionPolicy) {
    ...
    if (
      !proposedHandlePosition || (*proposedHandlePosition == m_dragState.currentHandlePosition &&
...
/**
 * Picks a handle position for the current input state. The returned point is a handle position and
 * not a hit position, so it must be corrected by the handle offset if the offset is not zero.
 */
using DragHandlePicker = std::function<std::optional<vm::vec3>(const InputState&)>;

/**
 * Returns a drag handle picker that picks a point on a line. The given line should be based on the
 * initial handle position and not the hit point, i.e., it should contain the handle position.
 */
DragHandlePicker makeLineHandlePicker(const vm::line3& line, const vm::vec3& handleOffset);

/**
 * Returns a drag handle picker that picks a point on a plane. The given plane should be based on
 * the initial handle position and not the hit point, i.e. it should contain the handle position.
 */
DragHandlePicker makePlaneHandlePicker(const vm::plane3& plane, const vm::vec3& handleOffset);

/**
 * Returns a drag handle picker that picks a point on a circle. The distance of the returned point
 * and the given center is always equal to the given adius.
 */
...
/**
 * Returns a drag handle picker that picks a point on a surface. The surface is determined by the
 * given hit filter. It is used to find a hit in the input state's pick result, and that hit's hit
 * point is returned, corrected by the given handle offset.
 */
...
/**
 * Returns a snapper function that snaps the proposed handle position such that the distance to the
 * initial handle position (passed in the drag state) is snapped to the grid.
 */
...
/**
 * Returns a snapper function that snaps the proposed handle position to the closest point on the
 * given line such that the distance between that point and the initial handle position is a
 * multiple of the grid size. If the initial handle position is not on the line itself, it is
 * orthogonally projected onto the line.
 */
DragHandleSnapper makeRelativeLineHandleSnapper(const Grid& grid, const vm::line3& line);

/**
 * Returns a snapper function that snaps the proposed handle position to the closest point on the
 * given line such that any of its components is a multiple of the grid size.
 */
DragHandleSnapper makeAbsoluteLineHandleSnapper(const Grid& grid, const vm::line3& line);

/**
 * Returns a snapper function that snaps the proposed handle position to a point on a circle such
 * that the angle between the vectors A and B is a multiple of the given snap angle. Thereby, vector
 * A = proposed handle position - center and vector B = initial handle position - center.
 *
 *         *   *
 ...
 *
 * In this example, the snap angle is 45, so the angle between the initial handle position and the
 * proposed handle position is snapped to 90°.
 */
DragHandleSnapper makeCircleHandleSnapper(
  const Grid& grid, FloatType snapAngle, const vm::vec3& center, const vm::vec3& normal,
  FloatType radius);

/**
 * Returns a handle proposer that proposes the position of the first brush face hit in the current
 * pick result, snapped to the grid projected onto that face.
 */
