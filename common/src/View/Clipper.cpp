...
namespace TrenchBroom {
    namespace View {
        ...
        Clipper::ClipPoints::ClipPoints(const ClipHandlePoints& handlePoints) :
        m_valid(false) {
        ...
        void Clipper::setPoints(const Vec3& p1, const Model::BrushFace* face, const Renderer::Camera& camera) {
            ...
            addClipPoint(p1, face, camera);
            addClipPoint(p2, face, camera);
            addClipPoint(p3, face, camera);
        }
        ...
        void Clipper::addClipPoint(const Vec3& position, const Model::BrushFace* face, const Renderer::Camera& camera) {
            m_handlePoints.addPoint(position, getNormals(position, face));
            updateClipPoints();
            setClipPlaneNormal(camera);
        }
        ...
        void Clipper::reset() {
            m_handlePoints.deleteAllPoints();
            m_clipPoints = ClipPoints(m_handlePoints);
        }
            ...
            if (m_clipPoints.valid()) {
                ...
                for (bIt = brushes.begin(), bEnd = brushes.end(); bIt != bEnd; ++bIt) {
                    ...
                    if (frontBrush->clip(worldBounds, frontFace))
                        result.frontBrushes[parent].push_back(frontBrush);
                    else
                        delete frontBrush;
                    
                    Model::Brush* backBrush = brush->clone(worldBounds);
                    if (backBrush->clip(worldBounds, backFace))
                        result.backBrushes[parent].push_back(backBrush);
                    else
                        delete backBrush;
                }
            } else {
        ...
        void Clipper::setClipPlaneNormal(const Renderer::Camera& camera) {
            if (m_clipPoints.valid()) {
                ...
                if (plane.pointStatus(camera.position()) == Math::PointStatus::PSInside) {
                    if (plane.normal.dot(camera.right()) < 0.0)
                        m_clipPoints.invertPlaneNormal();
                } else {
                    if (plane.normal.dot(camera.direction()) > 0.0)
                        m_clipPoints.invertPlaneNormal();
                }
            }
        }
