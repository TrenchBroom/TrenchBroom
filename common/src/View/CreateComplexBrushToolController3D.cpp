...
private:
  void updatePolyhedron(
    ...
    const auto theMin =
      vm::swizzle(grid.snapDown(vm::min(initialHandlePosition, currentHandlePosition)), axis);
    const auto theMax =
      vm::swizzle(grid.snapUp(vm::max(initialHandlePosition, currentHandlePosition)), axis);
  ...
  if (!polyhedron.empty()) {
    ...
    if (polyhedron.polygon() && inputState.modifierKeysDown(ModifierKeys::MKShift)) {
      ...
      if (hit.isMatch()) {
        ...
        auto pos3f = kdl::vec_transform(pos3, [](const auto& pos) {
          return vm::vec3f{pos};
        });
