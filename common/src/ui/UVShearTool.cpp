...
std::vector<vm::vec2f> getEdgeVectorsUV(const UVViewHelper& helper)
{
  ...
  {
    const auto toUV = helper.face()->toUVCoordSystemMatrix(
...
float snapShearFactors(
  const float factor, const vm::axis::type axis, const UVViewHelper& helper)
{
  const auto edgeVectors = getEdgeVectorsUV(helper);
  const auto snappedFactors = getSnappedShearFactors(edgeVectors, axis);

  const auto absDiff = [&](const auto& x) {
    return vm::abs(vm::abs(x) - vm::abs(factor));
  };
  ...
  return it != snappedFactors.end() && absDiff(*it) < 0.08f ? *it : factor;
}

vm::vec2f snapShearFactors(const vm::vec2f& factors, const UVViewHelper& helper)
{
  return vm::vec2f{
    snapShearFactors(factors.x(), vm::axis::x, helper),
    snapShearFactors(factors.y(), vm::axis::y, helper),
  };
}
...
public:
  ...
  bool update(const InputState& inputState) override
  {
    ...
    const auto snappedFactors =
      selectShearFactors(snapShearFactors(factors, m_helper), m_selector);
