  ...
  std::shared_ptr<mdl::Game> game, const mdl::MapFormat mapFormat)
{
  ...
  auto& map = m_document->map();
  return map.create(mapFormat, MapDocument::DefaultWorldBounds, game)
  ...
  const std::filesystem::path& path)
{
  ...
  return map.load(mapFormat, MapDocument::DefaultWorldBounds, game, path)
         | kdl::transform([&]() {
...
void MapFrame::revertDocument()
{
  ...
  {
    const auto mapFormat = map.world()->mapFormat();
    const auto game = map.game();
    const auto path = map.path();
    map.load(mapFormat, MapDocument::DefaultWorldBounds, game, path)
      | kdl::transform_error(
        [&](auto e) { logger().error() << "Failed to revert document: " << e.msg; });
  }
}
...
void MapFrame::dropEvent(QDropEvent* event)
{
  ...
  const auto newWadPathsStr = kdl::str_join(
    ...
    ";");
  map.setProperty(*wadPropertyKey, newWadPathsStr);
