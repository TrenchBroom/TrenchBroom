  ...
  const PropertyDefinition& definition)
{
  ...
  return std::visit(
    kdl::overload(
      ...
      [](const ColorPropertyType& value) -> std::optional<std::string> {
        return value.defaultValue | kdl::optional_transform([](const ColorValue cv) {
                 return std::visit(
                   kdl::overload(
                     [](const Color3f& c) {
                       return fmt::format(
