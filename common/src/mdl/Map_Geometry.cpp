      ...
      return std::function{[&]() {
        return node->accept(kdl::overload(
          ...
          [&](mdl::BrushNode* brushNode) -> TransformResult {
            ...
            alignmentLock
            || (containingGroup && containingGroup->closed() && mdl::collectLinkedNodes({world()}, *brushNode).size() > 1);
...
bool Map::snapVertices(const double snapTo)
{
  ...
  const bool applyAndSwapSuccess = mdl::applyAndSwap(
    *this,
    "Snap Brush Vertices",
    allSelectedBrushes,
    mdl::collectContainingGroups(kdl::vec_static_cast<mdl::Node*>(allSelectedBrushes)),
    kdl::overload(
...
bool Map::extrudeBrushes(const std::vector<vm::polygon3d>& faces, const vm::vec3d& delta)
{
  ...
  return mdl::applyAndSwap(
    *this,
    "Resize Brushes",
    nodes,
    mdl::collectContainingGroups(nodes),
    kdl::overload(
      [](mdl::Layer&) { return true; },
      [](mdl::Group&) { return true; },
      [](mdl::Entity&) { return true; },
