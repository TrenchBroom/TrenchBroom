...
el::ExpressionNode LegacyModelDefinitionParser::parseStaticModelDefinition()
{
  ...
  auto map = el::MapType{};
  map[ModelSpecificationKeys::Path] = el::Value(token.data());
  ...
  if (!indices.empty())
  {
    map[ModelSpecificationKeys::Skin] = el::Value(indices[0]);
    if (indices.size() > 1)
    {
      map[ModelSpecificationKeys::Frame] = el::Value(indices[1]);
    }
  }
  ...
  ParserStatus& status)
{
  ...
  auto map = std::map<std::string, el::ExpressionNode>{
    {ModelSpecificationKeys::Path, parseNamedValue("pathKey")},
  };
  ...
  if (!token.hasType(MdlToken::CParenthesis))
  {
    do
    {
      ...
      {
        map.emplace(ModelSpecificationKeys::Skin, parseNamedValue("skinKey"));
      }
      else if (kdl::ci::str_is_equal("frameKey", token.data()))
      {
        map.emplace(ModelSpecificationKeys::Frame, parseNamedValue("frameKey"));
      }
      else
      {
        const auto msg =
