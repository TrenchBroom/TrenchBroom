...
PropertyDefinition DefParser::parseSpawnflags()
{
  ...
  while (token.hasType(DefToken::Word | DefToken::Minus))
  {
    ...
    flags.push_back(PropertyValueTypes::Flag{value, std::move(name), ""});
    token = m_tokenizer.peekToken();
  }

  return {
    EntityPropertyKeys::Spawnflags, PropertyValueTypes::Flags{std::move(flags)}, "", ""};
}
...
PropertyDefinition DefParser::parseChoicePropertyDefinition()
{
  ...
  while (token.type() == DefToken::OParenthesis)
  {
    ...
    options.push_back(
      PropertyValueTypes::ChoiceOption{std::move(value), std::move(name)});
  ...
  return {std::move(propertyKey), PropertyValueTypes::Choice{std::move(options)}, "", ""};
}

ModelDefinition DefParser::parseModelDefinition(ParserStatus& status)
{
  m_tokenizer.nextToken(DefToken::OParenthesis);

  return mdl::parseModelDefinition(m_tokenizer, status, DefToken::CParenthesis)
