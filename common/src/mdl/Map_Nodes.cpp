...
std::vector<mdl::Node*> Map::addNodes(
...
bool Map::reparentNodes(const std::map<Node*, std::vector<Node*>>& nodesToAdd)
{
  ...
  {
    auto* newParentLayer = findContainingLayer(newParent);

    const auto nodesToDowngrade = collectNodesAndDescendants(
      nodes, [&](Object* node) { return node->containingLayer() != newParentLayer; });
