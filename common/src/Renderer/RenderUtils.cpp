...
vm::vec3f gridColorForTexture(const Assets::Texture* texture) {
  ...
  if (
    (texture->averageColor().r() + texture->averageColor().g() + texture->averageColor().b()) /
      3.0f >
    0.50f) {
...
std::vector<vm::vec3f> circle2D(
  const float radius, const vm::axis::type axis, const float startAngle, const float angleLength,
...
size_t midPoint(
  std::vector<vm::vec3f>& vertices, MidPointCache& cache, const size_t index1,
  const size_t index2) {
