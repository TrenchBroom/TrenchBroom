...
PerspectiveCamera::PerspectiveCamera(
  const float fov, const float nearPlane, const float farPlane, const Viewport& viewport,
...
void PerspectiveCamera::doValidateMatrices(
  ...
  projectionMatrix =
    vm::perspective_matrix(zoomedFov(), nearPlane(), farPlane(), viewport.width, viewport.height);
...
void PerspectiveCamera::getFrustumVertices(const float size, vm::vec3f (&verts)[4]) const {
  ...
  verts[0] = position() + (direction() * nearPlane() + frustum.y() * up() - frustum.x() * right()) /
                            nearPlane() * size; // top left
  verts[1] = position() + (direction() * nearPlane() + frustum.y() * up() + frustum.x() * right()) /
                            nearPlane() * size; // top right
  verts[2] = position() + (direction() * nearPlane() - frustum.y() * up() + frustum.x() * right()) /
                            nearPlane() * size; // bottom right
  verts[3] = position() + (direction() * nearPlane() - frustum.y() * up() - frustum.x() * right()) /
