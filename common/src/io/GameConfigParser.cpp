...
Result<mdl::GameConfig> GameConfigParser::parse()
{
  ...
  try
  {
    return parseConfigFile()
           | kdl::and_then([&](const auto& expression) -> Result<mdl::GameConfig> {
               ...
               return expression.evaluate(evaluationContext, trace)
  ...
  {'icon': 'String', 'experimental': 'Boolean', 'faceattribs': 'Map', 'tags': 'Map', 'softMapBounds': 'String'}
                          ...
                          auto mapFormatConfigs =
                            parseMapFormatConfigs(root["fileformats"], trace);
                          auto fileSystemConfig =
                            parseFileSystemConfig(root["filesystem"], trace);
                          auto materialConfig =
                            parseMaterialConfig(root["materials"], trace);
                          ...
                          auto softMapBounds =
                            parseSoftMapBounds(root["softMapBounds"], trace);
