...
{
  expect(Quake3ShaderToken::OBrace, m_tokenizer.skipAndNextToken(Quake3ShaderToken::Eol));
  auto token = m_tokenizer.skipAndPeekToken(Quake3ShaderToken::Eol);
  expect(
  ...
  expect(Quake3ShaderToken::CBrace, m_tokenizer.skipAndNextToken(Quake3ShaderToken::Eol));
}
...
{
  const auto token = expect(
    Quake3ShaderToken::String, m_tokenizer.skipAndNextToken(Quake3ShaderToken::Eol));
...
void Quake3ShaderParser::parseBodyEntry(
  mdl::Quake3Shader& shader, ParserStatus& /* status */)
{
  auto token = m_tokenizer.skipAndNextToken(Quake3ShaderToken::Eol);
  ...
  else if (kdl::ci::str_is_equal(key, "cull"))
  {
    ...
    {
      shader.culling = mdl::Quake3Shader::Culling::Front;
    }
    else if (kdl::ci::str_is_equal(value, "back"))
    {
      shader.culling = mdl::Quake3Shader::Culling::Back;
    }
    else if (
      kdl::ci::str_is_equal(value, "none") || kdl::ci::str_is_equal(value, "disable"))
    {
      shader.culling = mdl::Quake3Shader::Culling::None;
    }
  }
  else
  {
  ...
  mdl::Quake3ShaderStage& stage, ParserStatus& status)
{
  ...
  else if (kdl::ci::str_is_equal(key, "blendFunc"))
  {
    ...
    else
    {
      ...
      {
        stage.blendFunc.srcFactor = mdl::Quake3ShaderStage::BlendFunc::One;
        stage.blendFunc.destFactor = mdl::Quake3ShaderStage::BlendFunc::One;
      }
      else if (kdl::ci::str_is_equal(param1, "filter"))
      {
        stage.blendFunc.srcFactor = mdl::Quake3ShaderStage::BlendFunc::DestColor;
        stage.blendFunc.destFactor = mdl::Quake3ShaderStage::BlendFunc::Zero;
      }
      else if (kdl::ci::str_is_equal(param1, "blend"))
      {
        stage.blendFunc.srcFactor = mdl::Quake3ShaderStage::BlendFunc::SrcAlpha;
        stage.blendFunc.destFactor = mdl::Quake3ShaderStage::BlendFunc::OneMinusSrcAlpha;
      }
      else
      {
