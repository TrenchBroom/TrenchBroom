...
public:
  ...
  Result<mdl::EntityModelData> load()
  {
    try
    {
      auto scene = Scene{};
...
private: // parsing
  ...
  Result<mdl::EntityModelData> buildModelData(const Scene& scene) const
  {
...
Result<mdl::EntityModelData> loadAseModel(
  std::string name, std::string_view str, LoadMaterialFunc loadMaterial, Logger& logger)
{
