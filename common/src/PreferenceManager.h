...
public:
  ...
  template <typename T> Preference<T>& dynamicPreference(const IO::Path& path, T&& defaultValue) {
    ...
    auto* pref = dynamic_cast<Preference<T>*>(prefBase);
      ...
      pref != nullptr, ("Preference " + path.asString() + " must be of the expected type").c_str());
    return *pref;
  }
...
using PreferencesResult = kdl::result<
  ...
  PreferenceErrors::NoFilePresent, PreferenceErrors::JsonParseError,
