...
static std::optional<T> evaluateValueOp(
  const std::optional<T>& oldValue, const T oldValueFallback, const std::optional<T>& newValue,
...
bool ChangeBrushFaceAttributesRequest::evaluate(BrushFace& brushFace) const {
  ...
  result |=
    attributes.setRotation(evaluateValueOp(attributes.rotation(), m_rotation, m_rotationOp));
  ...
  result |= attributes.setSurfaceContents(evaluateFlagOp(
    attributes.surfaceContents(), brushFace.resolvedSurfaceContents(), m_contentFlags,
    m_contentFlagsOp));
  ...
  result |= attributes.setColor(
    evaluateValueOp(attributes.color(), brushFace.resolvedColor(), m_colorValue, m_colorValueOp));
