...
namespace TrenchBroom {
    namespace Model {
        ...
        std::string Entity::spritePath() const {
            if (const auto* pointDefinition = dynamic_cast<const Assets::PointEntityDefinition*>(m_definition.get())) {
                ...
                return pointDefinition->spriteDefinition().spritePath(variableStore);
            }
            return std::string();
        }

        const Assets::Texture* Entity::sprite() const {
            return m_sprite;
        }

        void Entity::setSprite(const Assets::Texture* sprite) {
            m_sprite = sprite;
        }
