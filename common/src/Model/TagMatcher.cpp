...
void TextureTagMatcher::enable(TagMatcherCallback& callback, MapFacade& facade) const {
  ...
  std::copy_if(
    std::begin(allTextures), std::end(allTextures), std::back_inserter(matchingTextures),
    [this](auto* texture) {
  ...
  if (matchingTextures.empty()) {
    ...
    const auto options = kdl::vec_transform(matchingTextures, [](const auto* current) {
      return current->name();
    });
...
FlagsTagMatcher::FlagsTagMatcher(
  const int flags, GetFlags getFlags, SetFlags setFlags, SetFlags unsetFlags,
...
ContentFlagsTagMatcher::ContentFlagsTagMatcher(const int i_flags)
  : FlagsTagMatcher(
      ...
      [](const BrushFace& face) {
        return face.resolvedSurfaceContents();
      },
...
SurfaceFlagsTagMatcher::SurfaceFlagsTagMatcher(const int i_flags)
  : FlagsTagMatcher(
      i_flags,
      [](const BrushFace& face) {
        return face.resolvedSurfaceFlags();
      },
      [](ChangeBrushFaceAttributesRequest& request, const int flags) {
...
void EntityClassNameTagMatcher::enable(TagMatcherCallback& callback, MapFacade& facade) const {
  ...
  std::copy_if(
    std::begin(allDefinitions), std::end(allDefinitions), std::back_inserter(matchingDefinitions),
  ...
  std::sort(
    std::begin(matchingDefinitions), std::end(matchingDefinitions),
  ...
  if (matchingDefinitions.empty()) {
    ...
    const auto options = kdl::vec_transform(matchingDefinitions, [](const auto* current) {
      return current->name();
    });
    const auto index = callback.selectOption(options);
