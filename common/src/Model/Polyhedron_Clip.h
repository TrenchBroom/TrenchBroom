...
typename Polyhedron<T, FP, VP>::ClipResult Polyhedron<T, FP, VP>::clip(
  ...
  // The basic idea is now to split all faces which are intersected by the given plane so that the
  // polyhedron can be separated into two halves such that no face has vertices on opposite sides of
  // the plane. Sometimes building a seam fails due to floating point imprecisions. In that case,
  // intersectWithPlane throws a NoSeamException which we catch here.
  try {
    const Seam seam = intersectWithPlane(plane);

    // We construct a seam along those edges which are completely inside the plane and delete the
    // half of the polyhedron that is above the plane. The remaining half is an open polyhedron (one
    // face is missing) which is below the plane.
    split(seam);
...
std::optional<typename Polyhedron<T, FP, VP>::ClipResult::FailureReason> Polyhedron<
  T, FP, VP>::checkIntersects(const vm::plane<T, 3>& plane) const {
  std::size_t above = 0u;
  std::size_t below = 0u;
  std::size_t inside = 0u;

  for (const Vertex* currentVertex : m_vertices) {
    const vm::plane_status status =
      plane.point_status(currentVertex->position(), vm::constants<T>::point_status_epsilon());
    switch (status) {
...
typename Polyhedron<T, FP, VP>::Seam Polyhedron<T, FP, VP>::intersectWithPlane(
  ...
  if (initialEdge == nullptr) {
    // No initial edge to split could be found. The brush is likely invalid, but wasn't recognized
    // as such due to floating point inaccuracies.
  ...
  // Now we split the face to which this initial half edge belongs. The call returns the newly
  // inserted edge that connects the (possibly newly inserted) vertices which are now inside of the
  // plane.
  ...
  // The destination of that edge is the first vertex which we encountered (or inserted) which is
  // inside the plane. This is where our algorithm must stop. When we encounter that vertex again,
  // we have completed the intersection and the polyhedron can now be split in two along the
  // computed seam.
  Vertex* stopVertex = currentEdge->destination();
  do {
    // First we find the next face that is either split by the plane or which has an edge completely
    // in the plane.
    currentEdge = findNextIntersectingEdge(currentEdge, plane);

    // If no edge could be found, then we cannot build a seam because the plane is barely touching
    // the polyhedron.
    if (currentEdge == nullptr) {
      throw NoSeamException(std::move(splitFaces));
    }

    // Now we split that face. Again, the returned edge connects the two (possibly inserted)
    // vertices of that face which are now inside the plane.
    std::tie(currentEdge, faceWasSplit) = intersectWithPlane(currentEdge, plane);

    // Build a seam while intersecting the polyhedron by remembering the edges we just inserted. To
    // ensure that the seam edges are correctly oriented, we check that the current edge is the
    // second edge, as the current edge belongs to the faces that we are going to clip away.
    Edge* seamEdge = currentEdge->edge();
    seamEdge->makeSecondEdge(currentEdge);
...
template <typename T, typename FP, typename VP>
typename Polyhedron<T, FP, VP>::HalfEdge* Polyhedron<T, FP, VP>::findInitialIntersectingEdge(
  const vm::plane<T, 3>& plane) const {
  for (const Edge* currentEdge : m_edges) {
    HalfEdge* halfEdge = currentEdge->firstEdge();
    const vm::plane_status os =
      plane.point_status(halfEdge->origin()->position(), vm::constants<T>::point_status_epsilon());
    ...
    if (os == vm::plane_status::inside && ds == vm::plane_status::inside) {
      ...
      // half edge, which is either the current one or its twin. Since the returned half edge is
      // supposed to be clipped away, we must examine the destination of its successor(s). If that
      // is below the plane, we return the twin, otherwise we return the half edge.
      ...
      while (ss == vm::plane_status::inside && nextEdge != halfEdge) {
        // Due to floating point imprecision, we might run into the case where the successor's
        // destination is still considered "inside" the plane. In this case, we consider the
        // successor's successor and so on until we find an edge whose destination is not inside the
        // plane.
...
template <typename T, typename FP, typename VP>
std::tuple<typename Polyhedron<T, FP, VP>::HalfEdge*, bool> Polyhedron<
  T, FP, VP>::intersectWithPlane(HalfEdge* firstBoundaryEdge, const vm::plane<T, 3>& plane) {

  // Starting at the given edge, we search the boundary of the incident face until we find an edge
  // that is either split in two by the given plane or where its origin is inside it. In the first
  // case, we split the found edge by inserting a vertex at the position where the plane intersects
  // the edge. We remember the half edge starting at the newly inserted vertex as the seam origin or
  // destination, depending on whether it's the first or second such edge we have found. In the
  // second case (the edge's origin is inside the plane), we just store the half edge as either the
  // seam origin or destination. In the end, we have two vertices, identified by half edges
  // belonging to the currently treated face, which lie inside the plane. If these two vertices
  // aren't already connected by an edge, we split the current face in two by inserting a new edge
  // from the origin to the destination vertex. Finally we must decide where to continue our search,
  // that is, we find a face that is incident to the destination vertex such that it is split by the
  // given plane. We return the half edge of that face's boundary which starts in the destination
  // vertex so that the search can continue there.
  ...
  do {
    ...
    if (os == vm::plane_status::inside) {
      ...
      (os == vm::plane_status::below && ds == vm::plane_status::above) ||
      (os == vm::plane_status::above && ds == vm::plane_status::below)) {
      // We have to split the edge and insert a new vertex, which will become the origin or
      // destination of the new seam edge.
      ...
      assert(
        plane.point_status(newVertex->position(), vm::constants<T>::point_status_epsilon()) ==
        vm::plane_status::inside);

      m_vertices.push_back(newVertex);

      // The newly inserted vertex will be reexamined in the next loop iteration as it is now
  ...
  if (seamDestination->next() == seamOrigin) {
    ...
    // If the origin and the destination are not already connected by an edge, we must split the
    // current face and insert an edge between them. The newly created faces are supposed to be
    // above the given plane, so we have to consider whether the destination of the seam origin edge
...
/*
 Searches all edges leaving searchFrom's destination for an edge that is intersected by the given
 plane.
 */
template <typename T, typename FP, typename VP>
typename Polyhedron<T, FP, VP>::HalfEdge* Polyhedron<T, FP, VP>::findNextIntersectingEdge(
  ...
  do {
    ...
    // Select two vertices that form a triangle (of an adjacent face) together with currentEdge's
    // origin vertex. If either of the two vertices is inside the plane or if they lie on different
    // sides of it, then we have found the next face to handle.
