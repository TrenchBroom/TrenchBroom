...
Result<Value> evaluate(const std::string& expression, const MapType& variables = {})
{
...
{
  const auto context = EvaluationContext{VariableTable{variables}};
  return io::ELParser::parseStrict(expression).value().tryEvaluate(context);
}
...
TEST_CASE("Expression")
{
  ...
  SECTION("Operators")
  {
    using T = std::tuple<std::string, std::variant<Value, Error>>;
    ...
    {
      CHECK(evaluate(expression).is_error());
    }
  }
