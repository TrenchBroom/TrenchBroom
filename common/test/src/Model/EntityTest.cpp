...
TEST_CASE("EntityTest.addOrUpdateProperty") {
  ...
  SECTION("Setting a new property to protected by default") {
    ...
    CHECK_THAT(
      entity.protectedProperties(), Catch::UnorderedEquals(std::vector<std::string>{"newKey"}));
...
TEST_CASE("EntityTest.setOrigin") {
  ...
  SECTION("Updates cached model transformation") {
    ...
    REQUIRE(
      ...
      vm::translation_matrix(vm::vec3{1, 2, 3}) * vm::scaling_matrix(vm::vec3{2, 2, 2}));

    entity.setOrigin(config, vm::vec3{9, 8, 7});
    REQUIRE(
      entity.modelTransformation() ==
      vm::translation_matrix(vm::vec3{9, 8, 7}) * vm::scaling_matrix(vm::vec3{2, 2, 2}));
  }
}
...
TEST_CASE("EntityTest.transform") {
  ...
  SECTION("Updates cached model transformation") {
    ...
    CHECK(
      entity.modelTransformation() ==
      vm::translation_matrix(vm::vec3{8, 7, 6}) * vm::scaling_matrix(vm::vec3{2, 2, 2}));
  }
}
} // namespace Model
} // namespace TrenchBroom
