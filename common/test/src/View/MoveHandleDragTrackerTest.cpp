...
namespace vm {
    ...
    private:
        ...
        FloatType m_epsilon;
    public:
        explicit approx(const DragState value, const FloatType epsilon) : m_value(value), m_epsilon(epsilon) { assert(epsilon >= FloatType(0)); }
        ...
        friend bool operator==(const DragState& lhs, const approx<DragState>& rhs) {
            return lhs.initialHandlePosition == approx<vec3>{rhs.m_value.initialHandlePosition, rhs.m_epsilon} 
                && lhs.currentHandlePosition == approx<vec3>{rhs.m_value.currentHandlePosition, rhs.m_epsilon}
    ...
    namespace View {
        ...
        static auto makeMoveTracker(const InputState& inputState, const vm::vec3& initialHandlePosition, const vm::vec3& handleOffset) {
            ...
            auto makeDragHandleSnapper = [](const InputState&, const SnapMode) {
                return [](const InputState&, const DragState&, const vm::vec3& currentHitPosition) { return currentHitPosition; };
            };
            ...
            return HandleDragTracker<MoveHandleDragDelegate<decltype(delegate)>>{MoveHandleDragDelegate{std::move(delegate)}, inputState, initialHandlePosition, handleOffset};
        }
        ...
        TEST_CASE("MoveDragTracker.constructor") {
            ...
            GIVEN("A 3D camera") {
                ...
                WHEN("A tracker is created without any modifier keys pressed") {
                    auto tracker = makeMoveTracker(makeInputState(vm::vec3{0, 0, 64}, vm::vec3{0, 1, -1}, camera3d), initialHandlePosition, handleOffset);
                    
                    THEN("The tracker has set the initial and current handle positions correctly") {
                        CHECK(tracker.dragState() == DragState{
                ...
                WHEN("A tracker is created with the alt modifier pressed") {
                    auto tracker = makeMoveTracker(makeInputState(vm::vec3{0, 0, 64}, vm::vec3{0, 1, -1}, camera3d, ModifierKeys::MKAlt), initialHandlePosition, handleOffset);

                    THEN("The tracker is using a vertical hit finder") {
                        // we check this indirectly by observing how the move handle position changes when dragging
                        REQUIRE(tracker.drag(makeInputState(vm::vec3{16, 16, 64}, vm::vec3{0, 1, -1}, camera3d)));
                        CHECK(tracker.dragState() == vm::approx{DragState{
                            initialHandlePosition,
                            vm::vec3{0, 64, 16},
                            handleOffset
                        }});
            ...
            GIVEN("A 2D camera") {
                ...
                WHEN("A tracker is created without any modifier keys pressed") {
                    auto tracker = makeMoveTracker(makeInputState(vm::vec3{0, 64, 64}, vm::vec3{0, 0, -1}, camera2d), initialHandlePosition, handleOffset);
                ...
                WHEN("A tracker is created with the alt modifier pressed") {
                    auto tracker = makeMoveTracker(makeInputState(vm::vec3{0, 0, 64}, vm::vec3{0, 1, -1}, camera2d, ModifierKeys::MKAlt), initialHandlePosition, handleOffset);
        ...
        TEST_CASE("MoveDragTracker.modifierKeyChange") {
            ...
            GIVEN("A tracker created with a 3D camera") {
                auto camera3d = Renderer::PerspectiveCamera{};
                auto tracker = makeMoveTracker(makeInputState(vm::vec3{0, 0, 64}, vm::vec3{0, 1, -1}, camera3d), initialHandlePosition, handleOffset);
                REQUIRE(tracker.dragState() == DragState{
                    initialHandlePosition,
                    initialHandlePosition,
                    handleOffset
                });
            ...
            GIVEN("A tracker created with a 3D camera") {
                ...
                auto tracker = makeMoveTracker(makeInputState(vm::vec3{0, 0, 64}, vm::vec3{0, 1, -1}, camera2d), initialHandlePosition, handleOffset);
                REQUIRE(tracker.dragState() == DragState{
                    initialHandlePosition,
                    initialHandlePosition,
                    handleOffset
