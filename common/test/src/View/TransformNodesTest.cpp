...
TEST_CASE_METHOD(MapDocumentTest, "TransformNodesTest.flip") {
  ...
  Model::BrushNode* brushNode1 = new Model::BrushNode(
    builder.createCuboid(vm::bbox3(vm::vec3(0.0, 0.0, 0.0), vm::vec3(30.0, 31.0, 31.0)), "texture")
      .value());
  Model::BrushNode* brushNode2 = new Model::BrushNode(
    builder.createCuboid(vm::bbox3(vm::vec3(30.0, 0.0, 0.0), vm::vec3(31.0, 31.0, 31.0)), "texture")
      .value());
  ...
  CHECK(
    brushNode1->logicalBounds() == vm::bbox3(vm::vec3(1.0, 0.0, 0.0), vm::vec3(31.0, 31.0, 31.0)));
  CHECK(
    brushNode2->logicalBounds() == vm::bbox3(vm::vec3(0.0, 0.0, 0.0), vm::vec3(1.0, 31.0, 31.0)));
}
...
TEST_CASE_METHOD(MapDocumentTest, "TransformNodesTest.translateLinkedGroup") {
  ...
  document->selectNode(linkedGroup);
  REQUIRE_THAT(
  ...
  auto getTexCoords =
    ...
    return kdl::vec_transform(face.vertexPositions(), [&](auto x) {
      return face.textureCoords(x);
    });
  };

  // Brushes in linked groups should have texture lock forced on
  CHECK(UVListsEqual(
    getTexCoords(brushNode1, vm::vec3::pos_z()), getTexCoords(linkedBrushNode, vm::vec3::pos_z())));
