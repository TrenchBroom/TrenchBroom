    ...
    namespace Model {
        ...
        public:
            ...
            inline void textureCoordinates(const Vec3f& vertex, Vec2f& result) const {
                result.x = ((vertex | m_textureInfo->sAxis) + m_textureInfo->sOffset) / m_textureInfo->texture->width();
                result.y = ((vertex | m_textureInfo->tAxis) + m_textureInfo->tOffset) / m_textureInfo->texture->height();
            }
